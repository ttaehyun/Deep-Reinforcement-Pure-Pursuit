...
class DrappEnv(gym.Env):
    def __init__(self):
        ...
        # 에피소드(랩) 데이터 저장을 위한 리스트
        self.episode_speeds = []
        self.episode_ctes = []
        self.episode_steerings = []
        self.episode_computation_times = []
        # 랩 완주 판단을 위한 하이퍼파라미터
        self.lap_completion_threshold = 5.0 # 시작점으로부터 이 거리(m) 내에 들어오면 완주로 간주
        self.min_lap_distance_ratio = 0.8 # 전체 경로의 80% 이상 주행해야 랩으로 인정
    ...
    def odom_cb(self, msg):
        ...
        if self.last_position:
            ...
            # print("Total Distance Traveled: %.2f meters" % self.total_distance_traveled)
    ...
    def reset(self, seed=None, options = None):
        ...
        if len(self.path.poses) > 0:
            if self.path_length == 0.0: # 최초 한 번만 경로 총 길이 계산
                self.path_length = self.ppControl.calculateTotalPathLength()
            print("Total Path Length: %.2f meters" % self.path_length)
            # 랩 시작 상태 초기화
            self.start_pose = self.path.poses[0].pose.position
            self.last_position = self.odom.pose.pose.position # 현재 위치로 초기화
            self.total_distance_traveled = 0.0
            self.lap_start_time = time.time()
            print("New lap started. Start position set to (%.2f, %.2f)" % (self.start_pose.x, self.start_pose.y))
        self.episode_speeds.clear()
        self.episode_ctes.clear()
        self.episode_steerings.clear()
        self.episode_computation_times.clear()
    ...
    def process_episode_results(self):
        if not self.lap_start_time or not self.episode_speeds:
            return
        ...
        avg_cte = np.mean(np.abs(self.episode_ctes)) # CTE는 절대값의 평균
        max_cte = np.max(np.abs(self.episode_ctes))
        steering_std = np.std(self.episode_steerings)
        avg_comp_time = np.mean(self.episode_computation_times) * 1000
        # 결과 출력 (pure_pursuit.py와 동일한 형식)
        rospy.loginfo("="*40)
        rospy.loginfo(f"LAP {self.lap_count} RESULTS")
    ...
    def step(self, action):
        ...
        self.episode_speeds.append(self.current_vel_ms) 
        self.episode_ctes.append(cte)
        self.episode_steerings.append(steering_angle)
       
        terminated = False
        # 랩 완주 여부 확인
        if self.start_pose:
            dist_to_start = math.sqrt((self.odom.pose.pose.position.x - self.start_pose.x)**2 + 
                                      (self.odom.pose.pose.position.y - self.start_pose.y)**2)
            # print("Distance to Start: %.2f meters" % dist_to_start)
            # print("Lap completed! Total Distance: %.2f meters" % self.total_distance_traveled)
            if dist_to_start < self.lap_completion_threshold and \
                self.total_distance_traveled > self.path_length * self.min_lap_distance_ratio:
                self.lap_count += 1
                rospy.loginfo(f"LAP {self.lap_count} COMPLETED.")
                self.process_episode_results() # 에피소드(랩) 결과 출력
                self.start_pose = self.path.poses[0].pose.position
                self.last_position = self.odom.pose.pose.position # 현재 위치로 초기화
                self.total_distance_traveled = 0.0
                self.lap_start_time = time.time()
